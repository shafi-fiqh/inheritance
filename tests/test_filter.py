...
def test_filter():
    ...
    :return:
    """
    casegen = CaseGenerator(config='config/family_config.csv',
                            filter = 'config/filter.yml',
                            filter_bool = True)
    casegen.generate_cases(n_types=4)
    casegen.save_cases(output=os.path.join('output', 'test.csv'),
                       chunk_size=10000)
    ...
    assert [x in inh for inh in inheritors for x in must_have], 'Filter failed, missing must have inheritors'
    assert [x not in inh for inh in inheritors for x in ignore], 'Filter failed ignored inheritors present'
