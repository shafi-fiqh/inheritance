...
def is_akdariyya(case: dict) -> bool:
    ...
    """
    cond1 = 'husband' in case
    cond2 = ('sister' in case and 'brother' not in case) or ('paternal_halfsister' in case and 'paternal_halfbrother' not in case)
...
def calculate_remainder_grandfather(case: dict) -> dict:
    ...
    :return:
    """
    scope = {x:case[x] for x in case if x not in ['father_of_father', 'brother', 'sister', 'sister_x2', 'paternal_halfsister', 'paternal_halfsister_x2']}
    fard_basic = [share for share in scope.values() if share in ['1/8', '1/4', '1/2', '1/6', '1/3', '2/3']]
