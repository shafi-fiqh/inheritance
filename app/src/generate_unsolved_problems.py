...
UNIVERSAL_HEIRS = ['son', 'son_of_son', 'father', 'father_of_father', 'brother', 'paternal_halfbrother',
                   'nephew_brother', 'nephew_paternal_halfbrother', 'fathers_brother', 'fathers_paternal_halfbrother',
                   'son_of_fathers_brother', 'son_of_fathers_paternal_halfbrother', 'atiq', 'mutiqa']
...
def generate_problems_lst(
    inheritors: list, must_haves: list, not_haves: list, n_types: int, grand_father_and_siblings: bool
):
    inheritors = [
    ...
    siblings = ['brother', 'paternal_halfbrother', 'sister', 'sister_x2',
                'paternal_halfsister', 'paternal_halfsister_x2']

    for case in tqdm.tqdm(
        generator,
        total=nCr(n=len(inheritors), r=n_types - len(must_haves)),
    ):
        if not grand_father_and_siblings and any([sibling in case for sibling in siblings]) and grandfather in case:
            continue
