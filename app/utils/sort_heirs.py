def order_dict_by_keys(inh_dict: dict) -> dict:
    ...
    fixed_inh = {key: value for key, value in inh_dict.items() if key != first_key and value != 'U'}
...
def order_inhs_according_to_sorted(sorted_inh: dict, unsorted_inh: dict) -> dict:
    ...
    ordered_dict = {key: unsorted_inh.get(key) for key in sorted_inh if key in sorted_inh}
    return ordered_dict 